      ...
      <div className="container p-6 text-white border-2 rounded-2xl flex">
        <ScrollArea className="h-[800px] w-[300px] flex-none">
          {episodes.map((episode) => (
            <div className="py-4 px-4 flex justify-between items-center" key={episode?.id}>
              <EpisodeInfo
                episode={episode!}
              ...
              />
              <Link className='hover:text-orange-400 hover:scale-105' href={`/episodes/${episode?.id}`}><IoIosArrowDropright size={30} /></Link>
            </div>
          ))}
        </ScrollArea>
